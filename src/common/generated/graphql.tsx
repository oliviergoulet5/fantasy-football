...
export type QueryPlayersArgs = {
  ictIndex?: Maybe<InputRangeDouble>;
...
export type PlayerDataQuery = (
  { __typename?: 'Query' }
  & { players: Array<(
    { __typename?: 'Player' }
    & Pick<Player, 'firstName' | 'lastName' | 'goalsScored' | 'assists' | 'ictIndex' | 'minutes' | 'redCards' | 'yellowCards'>
  )> }
);
...
export const PlayerDataDocument = gql`
    query PlayerData($assists: InputRange, $goalsScored: InputRange) {
  players(assists: $assists, goalsScored: $goalsScored) {
    firstName
    lastName
    goalsScored
    assists
...
/**
 * __usePlayerDataQuery__
 *
 * To run a query within a React component, call `usePlayerDataQuery` and pass it any options that fit your needs.
 * When your component renders, `usePlayerDataQuery` returns an object from Apollo Client that contains loading, error, and data properties
 * you can use to render your UI.
 *
 * @param baseOptions options that will be passed into the query, supported options are listed on: https://www.apollographql.com/docs/react/api/react-hooks/#options;
 *
 * @example
 * const { data, loading, error } = usePlayerDataQuery({
 *   variables: {
 *      assists: // value for 'assists'
 *      goalsScored: // value for 'goalsScored'
 *   },
 * });
 */
export function usePlayerDataQuery(baseOptions?: Apollo.QueryHookOptions<PlayerDataQuery, PlayerDataQueryVariables>) {
        const options = {...defaultOptions, ...baseOptions}
        return Apollo.useQuery<PlayerDataQuery, PlayerDataQueryVariables>(PlayerDataDocument, options);
      }
export function usePlayerDataLazyQuery(baseOptions?: Apollo.LazyQueryHookOptions<PlayerDataQuery, PlayerDataQueryVariables>) {
          const options = {...defaultOptions, ...baseOptions}
          return Apollo.useLazyQuery<PlayerDataQuery, PlayerDataQueryVariables>(PlayerDataDocument, options);
        }
export type PlayerDataQueryHookResult = ReturnType<typeof usePlayerDataQuery>;
export type PlayerDataLazyQueryHookResult = ReturnType<typeof usePlayerDataLazyQuery>;
export type PlayerDataQueryResult = Apollo.QueryResult<PlayerDataQuery, PlayerDataQueryVariables>;